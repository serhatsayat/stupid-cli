...
| D040 | M002/S04 | pattern | WorktreeManager replaces PRBuilder for slice lifecycle when injected | SliceRunner checks `context.worktreeManager` — if present, uses it for create/commit/merge/teardown; if absent, falls back to PRBuilder | WorktreeManager subsumes PRBuilder.createCommit and createBranch for slice-scoped work. Optional injection preserves backward compat — existing 23+ SliceRunner tests pass unchanged. PRBuilder stays for standalone commit/PR use. | No |
